...
def lstm_model(train_X):
    train_X = train_X.reshape(train_X.shape[0], train_X.shape[1], 1)
    model = tf.keras.Sequential()
    model.add(tf.keras.LSTM(256, input_shape=(train_X.shape[1], train_X.shape[2])))
    model.add(tf.keras.Dense(1))
    model.compile(optimizer='adam', loss='mse', metrics=['mae'])
    return model

def train_model(model, train_X, train_y, test_X, test_y, scale_y, h5file, target: str, JF_name: str,
    ...
    draw(history, test_X, test_y, scale_y, h5file, target, JF_name, FWQ_name,model_name)


def draw(history, test_X, test_y, scale_y, h5file, target: str,  JF_name: str, FWQ_name: str,model_name):
    ...
    plt.title(target + '_' + JF_name + "JF " + FWQ_name + 'fwq_' +model_name  +' Traing and Validation mse')
    plt.legend()
    plt.savefig("./pic/"+target + '_' + JF_name + "JF " + FWQ_name + 'fwq_' +model_name  +' Traing and Validation mse.png')
    #plt.show()
    ...
    plt.title(target + '_' + JF_name + "JF " + FWQ_name + 'fwq_' + model_name+' test'+' r2:'+str(round(r2_result,4))+' rmse:'+str(round(rmse_result,4)))
    plt.legend(loc='best')
    plt.savefig("./pic/"+target + '_' + JF_name + "JF " + FWQ_name + 'fwq_' + model_name+' test.png')
    #plt.show()
...
def test(test_X, test_y, h5file, target: str, JF_name: str,
    ...
    plt.plot([x for x in range(len(test_pred) - num, len(test_pred))], test_pred[len(test_pred) - num:len(test_pred)],
             linewidth=0.7, color='r', label='Test')
    plt.title(target +'_' + JF_name + "JF " + 'KT_' + str(FWQ_name) + '_' + model_name + ' test' + ' r2:' + str(
        round(r2_result, 4)) + ' rmse:' + str(round(rmse_result, 4)))
    plt.legend(loc='best')
    plt.savefig("./pic/test2_27_"+target + '_' + JF_name + "JF " + FWQ_name + 'fwq_' + model_name+' test.png')
    #plt.show()