...
def split_train_test(X, y,scaler_x_file,scaler_y_file):  # 需要归一化的分割数据集
...
def draw1(history, test_X, test_y, scale_y, h5file, target: str, JF_name: str, KT: int,model_name,next_num):
    ...
    plt.title(target + '_' + JF_name + "JF "+ 'KT_' + str(KT) +'_'+model_name+ str(next_num)+ ' Traing and Validation mse')
    plt.legend()
    plt.savefig('./pic/'+target + '_' + JF_name + "JF " + 'KT_' + str(KT)+'_'+model_name +str(next_num)+ ' Traing and Validation mse.png')
    #plt.show()
    ...
    plt.title(target + '_' + JF_name + "JF_" + str(KT) + "_KT_"+model_name+str(next_num) + ' test'+' r2:'+str(round(r2_result,4))+' rmse:'+str(round(rmse_result,4)))
    plt.legend(loc='best')
    plt.savefig('./pic/'+target + '_' + JF_name + "JF_" + str(KT) + "_KT_"+model_name+str(next_num) + ' test.png')
    #plt.show()

def test(test_X, test_y, h5file, target: str, JF_name: str,
                KT: int,model_name,scaler_x_file,scaler_y_file,next_num):
    model = tf.keras.models.load_model(h5file)  # 导出模型之后看误差图
    ...
    plt.plot([x for x in range(len(test_pred) - num, len(test_pred))], test_pred[len(test_pred) - num:len(test_pred)],
             linewidth=0.7, color='r', label='Test')
    plt.title(target + '_' + JF_name + "JF_" + str(KT) + "_KT_"+model_name+str(next_num) + ' test' + ' r2:' + str(
        round(r2_result, 4)) + ' rmse:' + str(round(rmse_result, 4)))
    plt.legend(loc='best')
    plt.savefig('./pic/'+target + '_' + JF_name + "JF_" + str(KT) + "_KT_"+model_name+str(next_num) + ' test.png')
    plt.show()

def test_model1(test_X, test_y, scaler_x_file,scaler_y_file, h5file, target: str, JF_name: str, KT: int,model_name,next_num):
    model = tf.keras.models.load_model(h5file)  # 导出模型之后看误差图
    scaler_x = load(scaler_x_file)
    scaler_y = load(scaler_y_file)
    test_pred = model.predict(scaler_x.transform(test_X))
    ...
    plt.title(target + '_' + JF_name + "JF_" + str(KT) + "_KT_"+model_name+str(next_num) + ' test'+' r2:'+str(round(r2_result,4))+' rmse:'+str(round(rmse_result,4)))
    plt.legend(loc='best')
    plt.savefig('./pic/test_2_27'+target + '_' + JF_name + "JF_" + str(KT) + "_KT_"+model_name+str(next_num) + ' test.png')
