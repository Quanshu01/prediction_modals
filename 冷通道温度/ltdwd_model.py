...
def kan_model(train_X):
    # train_X = train_X.reshape((train_X.shape[0], train_X.shape[1], 1))
    print("Before reshape, train_X shape:", train_X.shape)
    train_X = train_X.reshape((train_X.shape[0], train_X.shape[1], 1))
    print("After reshape, train_X shape:", train_X.shape)
    # train_X = train_X.reshape(train_X.shape[0], train_X.shape[1], 1)
    input_shape = (train_X.shape[1], train_X.shape[2])
...
def draw(history, test_X, test_y, scale_y, h5file, target, maxORmin, JF_name, FWQ_name, model_name, next_num):
    ...
    # Data True
    plt.plot([x for x in range(len(test_pred) - num, len(test_pred))], y[len(y) - num:len(y)], linewidth=1, color='b', label='True')
    # Data in Test
    plt.plot([x for x in range(len(test_pred) - num, len(test_pred))], test_pred[len(test_pred) - num:len(test_pred)], linewidth=0.7, color='r', label='Pred')
    ...
    print('完成画图')
    print('完成画图，存在','./pic/' + target + maxORmin + '_' + JF_name + "JF " + FWQ_name + 'fwq_' + model_name + str(next_num) +' test.png')
